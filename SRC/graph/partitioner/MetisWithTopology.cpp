...
// $Source: /usr/local/cvs/OpenSees/SRC/graph/partitioner/MetisWithTopology.cpp,v $
...
// Revision: A
//
// Description: This file contains the class definition for MetisWithTopology.
// MetisWithTopology is a type of GraphPartitioner which uses 'METIS - Unstructured
...
MetisWithTopology::MetisWithTopology(int numParts) 
:GraphNumberer(GraphNUMBERER_TAG_MetisWithTopology),
...
MetisWithTopology::MetisWithTopology(int Ptype,
             int Mtype,
             int coarsenTo,
             int Rtype,
             int IPtype,
...
MetisWithTopology::~MetisWithTopology()
{

}

bool
MetisWithTopology::setOptions(int Ptype,
                  int Mtype,
                  int coarsenTo,
                  int Rtype,
                  int IPtype)
...
bool
MetisWithTopology::checkOptions(void)
{
  ...
  if ((myPtype != 1) || (myPtype != 2)) {
    okFlag = false;
    opserr << "WARNING: MetisWithTopology::partition ";
...
bool
MetisWithTopology::setDefaultOptions(void)
{
  defaultOptions = true;
  return true;
}
...
int
MetisWithTopology::partition(Graph &theGraph, int numPart)
{
  ...
  int numEdge = theGraph.getNumEdge();
    //    opserr << " MetisWithTopology::partition --- numVertex: " << numVertex << " numEdge: "<<  numEdge << "\n";
  ...
  if (START_VERTEX_NUM == 0)
    ...
    opserr << "WARNING MetisWithTopology::partition - No partitioning done";
  ...
  for (int vertex = 0; vertex < numVertex; vertex++) {
    ...
    if (vertexPtr == 0) {
      opserr << "WARNING MetisWithTopology::partition - No partitioning done";
      opserr << " MetisWithTopology requires consequtive Vertex Numbering\n";
  ...
  if (myPtype == 1) {
    //opserr << " MetisWithTopology::partition PartGraphRecursive \n";
#ifdef _USE_METIS_5p1
    opserr << " MetisWithTopology::partition PartGraphRecursive not yet implemented for METIS 5.1 \n";
#else
    METIS_PartGraphRecursive(&numVertex, xadj, adjncy, vwgts, ewgts, &weightflag, &numbering, &numPart, options, &edgecut, partition);
#endif
  }
  else {
      //opserr << " MetisWithTopology::partition PartGraphKway w/ \n";
      /*
  ...
  //
  //opserr << " MetisWithTopology::partition returned ok \n";
...
int
MetisWithTopology::partitionGraph(int *nvtxs, int *xadj, int *adjncy, int *vwgt,
...
int
MetisWithTopology::partitionHexMesh(int* elmnts, int* epart, int* npart, int ne, int nn, int nparts, bool whichToUse)
{
  // which to use -> if true use edge cut partitinioning, else uses communication
  // based partition, standard aplications , uniformly mesh regions need use the edge-cut
  // first one works fine second is weird
...
const ID &
MetisWithTopology::number(Graph &theGraph, int lastVertex)
{
  // first we check that the options are valid
  ...
  if (theRefResult == 0) {
    opserr << "ERROR:  MetisWithTopology::number - Out of Memory\n";
    theRefResult = new ID(0);
    return *theRefResult;
  }

  if (checkOptions() == false) {
    opserr << "ERROR:  MetisWithTopology::number - chek options failed\n";
    return *theRefResult;
  }
  ...
  if (myPtype == 1)
    ...
    KMETIS(&numVertex, xadj, adjncy, vwgts, ewgts, &weightflag,
       &numPartitions, options, &numbering, &edgecut, partition);
  */
  opserr << "MetisWithTopology::number -2\n";
  ...
  opserr << "MetisWithTopology::number -3\n";
  // clean up the space and return
  delete [] options;
  delete [] partition;
  delete [] xadj;
  delete [] adjncy;
  opserr << "MetisWithTopology::number -4\n";
  return *theRefResult;
}


const ID &
MetisWithTopology::number(Graph &theGraph, const ID &lastVertices)
{
  return this->number(theGraph);
}

int
MetisWithTopology::sendSelf(int cTag, Channel &theChannel)
{
  return 0;
}

int
MetisWithTopology::recvSelf(int cTag, Channel &theChannel, FEM_ObjectBroker &theBroker)
{
  return 0;
}
